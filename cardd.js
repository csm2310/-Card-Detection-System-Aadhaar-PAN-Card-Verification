...
const panNumberElement = document.getElementById('panNumberResult'); // New element for displaying PAN number
...
function stopCamera() {
    ...
    panNumberElement.textContent = ""; // Reset the PAN number display
...
function capturePhoto() {
    ...
    // Preprocess image: Reduce contrast and brightness
...
function checkIdentityCard(text) {
    ...
    panNumberElement.textContent = ""; // Clear PAN number display
    ...
    if (panMatch) {
        cardType = "Detected: Permanent Account Number Card";
        panNumberElement.textContent = `PAN Number: ${panMatch[0]}`; // Display the detected PAN number
    } else {
        // Additional check for case variations and whitespace
        const cleanedText = text.replace(/\s/g, '').toUpperCase(); // Remove spaces and convert to uppercase
        const cleanedPanMatch = cleanedText.match(panRegex);
        if (cleanedPanMatch) {
            cardType = "Detected: Permanent Account Number Card";
            panNumberElement.textContent = `PAN Number: ${cleanedPanMatch[0]}`; // Display the detected PAN number
        }
    }

    // Update the UI with the detected card information
...
function detectCardType() {
    ...
    if (/^\d{12}$/.test(cardNumber)) {
        ...
        cardType = "Detected: Valid Pan Card";
        panNumberElement.textContent = `PAN Number: ${cardNumber}`; // Display the entered PAN number
    }
